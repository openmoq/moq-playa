 ...
 * for the CMAF Header's track is used), boxes after the mdat, and any 32-bit
 * or largesize pre-moof box as a genBox.
 *
 ...
 * @throws {LocmafFormatError} when the chunk has no single moof, no mdat, no
 *   traf for the track, no tfdt, a truncated box, or sample bytes outside the chunk.
 */
export function readCmafChunkSamples(chunk: Uint8Array, context: LocmafTrackContext): CmafChunkSamples {
    ...
    if (!top.slice(moofIndex + 1).some((b) => b.type === 'mdat')) throw new LocmafFormatError(S, moof.end, 'no mdat follows the moof');
    ...
    if (tfhd === undefined) throw new LocmafFormatError(S, traf.start, 'traf lacks tfhd');
    if (tfdt === undefined) throw new LocmafFormatError(S, traf.start, 'traf has no tfdt: samples cannot be placed in time');
    ...
    for (const trun of truns) {
        need(trun, 8);
        const version = chunk[trun.contentStart]!;
        ...
        for (let i = 0; i < n; i++) {
            ...
            if (cursor < 0 || cursor + size > chunk.length) {
                throw new LocmafFormatError(S, trun.start, `sample ${durations.length} lies outside the chunk`);
            }
...
/** The traf for the CMAF Header's track, or the only traf. */
function selectTraf(chunk: Uint8Array, moof: IsoBoxHeader, context: LocmafTrackContext): IsoBoxHeader {
    ...
    if (trafs.length === 1) return trafs[0]!;
    for (const traf of trafs) {
        const tfhd = childBoxes(chunk, traf.contentStart, traf.end, S).find((b) => b.type === 'tfhd');
        if (tfhd !== undefined && tfhd.end - tfhd.contentStart >= 8 && view.getUint32(tfhd.contentStart + 4) === context.trackId) return traf;
    }
    throw new LocmafFormatError(S, moof.start, `moof has ${trafs.length} trafs and none for track_ID ${context.trackId}`);
}
