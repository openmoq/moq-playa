 ...
 *
 * Uses PTS-anchored render scheduling: render times are computed from
 * CaptureTimestamps at decode OUTPUT (when frames arrive here), not at
 * decode INPUT (when the pipeline processes objects). This eliminates
 * startup stutter from async decode latency.
 *
...
export class CanvasRenderer implements VideoRendererLike {
  ...
  renderTick(nowUs: number): void {
    ...
    while (this.queue.length > 0) {
      ...
      if (entry.renderTimeUs <= nowUs && !rendered) {
        // On time — render (at most one frame per tick to prevent burst stutter)
        this.queue.shift();
  ...
  private scheduleLoop(): void {
    if (document.hidden) {
      ...
      this.intervalId = setInterval(() => {
        this.renderTick(performance.now() * 1000);
